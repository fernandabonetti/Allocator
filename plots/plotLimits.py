...
def main():
	...
	# plt.plot(steps, mem_min, lw=2, color='#fcaf3eff')
	# plt.plot(steps, mem_usage, lw=2, color='#ff0000ff')

	plt.plot(steps, cpu_max, lw=2)
	plt.plot(steps, cpu_min, lw=2, color='#fcaf3eff')
	plt.plot(steps, cpu_usage, lw=2, color='#ff0000ff')
	
	plt.margins(x=0)				#remove the ugly inner side margin 

	plt.ylabel('CPU (m)', fontsize=14)
	#plt.ylabel('Memory (MiB)', fontsize=14)
	...
	#plt.legend(loc='upper center', labels=['Max','Min','Usage'], fontsize=12, bbox_to_anchor=(0.5, 1.15), ncol=3)
